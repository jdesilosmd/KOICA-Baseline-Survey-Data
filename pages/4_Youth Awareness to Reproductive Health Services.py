...
df4 = df4[[col for col in df4.columns if not any(col.endswith(f".{num}") for num in range(1, 10))]]

# Normalize values: Replace underscores and capitalize strings
df4 = df4.applymap(lambda x: x.replace('_', ' ').capitalize() if isinstance(x, str) else x)
...
filtered_df4['City'] = filtered_df4['City'].str.strip().str.title()
if selected_sites:
    filtered_df4 = filtered_df4[filtered_df4['City'].isin(selected_sites)]
if selected_gender:
    filtered_df4 = filtered_df4[filtered_df4['Sex assigned at birth'].str.lower().isin(selected_gender)]

if age_group == "10-14":
    filtered_df4 = filtered_df4[
        (filtered_df4['Age as of last birthday'] >= 10) & (filtered_df4['Age as of last birthday'] <= 14)
    ]
elif age_group == "15-19":
    filtered_df4 = filtered_df4[
        (filtered_df4['Age as of last birthday'] >= 15) & (filtered_df4['Age as of last birthday'] <= 19)
    ]
...
def create_expander_and_graph(start_col, end_col, expander_title, graph_title, display_type):
    df_subset = filtered_df4.iloc[:, list(range(start_col, end_col + 1))]
...
expander_info = {
    4: {"title": filtered_df4.columns[4]},
    5: {"title": filtered_df4.columns[5]},
    6: {"title": filtered_df4.columns[6]},
    7: {"title": filtered_df4.columns[7]},
    36: {"title": filtered_df4.columns[36]},
    37: {"title": filtered_df4.columns[37]},
    38: {"title": filtered_df4.columns[38]},
    39: {"title": filtered_df4.columns[39]},
    40: {"title": filtered_df4.columns[40]},


}

# Create additional expanders for the specified ranges
create_expanders_from_info(expander_info, filtered_df4, display_type)
create_expander_and_graph(9, 17, "What types of contraception that you know are present in the facility?", "What types of contraception that you know are present in the facility?", display_type)
create_expander_and_graph(18, 30, "What are the types of health services that you know are present in the health facility?", "What are the types of health services that you know are present in the health facility?", display_type)
create_expander_and_graph(31, 36, "Are there any laboratory tests available for STI?", "Are there any laboratory tests available for STI?", display_type)
